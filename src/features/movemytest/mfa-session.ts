...
function getSecret() {
  ...
  if (!secret) {
    if (process.env.NODE_ENV === "production") throw new Error("TEST_SWAP_SECRET_KEY or AUTH_SECRET is required for MoveMyTest MFA challenges.");
    return "dtc-movemytest-mfa-dev-secret-change-me";
  }
  return secret;
}

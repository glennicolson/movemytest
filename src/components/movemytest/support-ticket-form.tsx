...
import { submitMoveMyTestSupportAction } from "@/features/movemytest/support-actions";

const categories = [
  { value: "GENERAL", label: "General question" },
  { value: "MATCH_HELP", label: "Help with a match" },
  { value: "DVSA_CALL", label: "DVSA phone process" },
  { value: "TECHNICAL", label: "Technical issue" },
  { value: "ACCOUNT", label: "Account// login" },
];

export function SupportTicketForm() {
  const [state, action, pending] = useActionState(submitMoveMyTestSupportAction, initialMoveMyTestActionState);
    ...
    <form action={action} className="mt-5 space-y-4">
      ...
      <label className="space-y-2 text-sm font-medium text-slate-800">
        ...
        <textarea
          ...
          placeholder="Tell MoveMyTest what you need help with. Include match ID or test centre if relevant."
        />
      </label>
